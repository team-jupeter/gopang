...
@app.get("/pollution/detect/{hash_id}")
async def detect_pollution(hash_id: str, method: str = "comprehensive"):
    """
    해시 오염 탐지
    
    Args:
        hash_id: 해시 ID
        method: statistical | pattern | comprehensive
    """
    try:
        detector = PollutionDetector()
        
        if method == "statistical":
            result = detector.detect_statistical_anomaly(hash_id)
        elif method == "pattern":
            result = detector.detect_pattern_anomaly(hash_id)
        else:
            result = detector.detect_comprehensive(hash_id)
...
@app.post("/pollution/scan-all")
async def scan_all_hashes():
    """모든 해시 일괄 검사"""
    try:
        detector = PollutionDetector()
        ...
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("SELECT hash_id FROM openhash_records")
        ...
        polluted = [r for r in results if r['is_polluted']]
        
        return {
            "total_scanned": len(results),
            "polluted_count": len(polluted),
            "clean_count": len(results) - len(polluted),
            "pollution_rate": round(len(polluted) / len(results) * 100, 2) if results else 0,
            "polluted_hashes": polluted
        }
...
from openhash.healing_mechanism import HealingMechanism, HealingStatus

@app.post("/healing/quarantine/{hash_id}")
async def quarantine_hash(hash_id: str):
    """해시 격리"""
    try:
        healer = HealingMechanism()
        result = healer.detect_and_quarantine(hash_id)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/healing/recover/{hash_id}")
async def recover_hash(hash_id: str):
    """네트워크에서 복원"""
    try:
        healer = HealingMechanism()
        result = healer.recover_from_network(hash_id)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/healing/verify/{hash_id}")
async def verify_healed(hash_id: str):
    """검증 및 치유 완료"""
    try:
        healer = HealingMechanism()
        result = healer.verify_and_heal(hash_id)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/healing/complete/{hash_id}")
async def complete_healing(hash_id: str):
    """전체 치유 워크플로우"""
    try:
        healer = HealingMechanism()
        result = healer.heal_complete_workflow(hash_id)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
